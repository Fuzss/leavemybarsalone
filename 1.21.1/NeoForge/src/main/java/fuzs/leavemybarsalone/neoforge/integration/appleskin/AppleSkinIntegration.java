...
public class AppleSkinIntegration {
    ...
    public static void init() {
        ...
        try {
            Class<?> renderOverlayTypeClazz = Class.forName(
                    "squeek.appleskin.client.HUDOverlayHandler$RenderOverlayType");
            MethodType renderExhaustionType = MethodType.methodType(void.class,
                    ExtendedGui.class,
                    GuiGraphics.class,
                    float.class,
                    int.class,
            ...
            MethodType renderFoodOrHealthOverlayType = MethodType.methodType(void.class,
                    ExtendedGui.class,
                    GuiGraphics.class,
                    float.class,
                    int.class,
                    int.class,
                    renderOverlayTypeClazz
            );
            Class<?> clazz = Class.forName("squeek.appleskin.client.HUDOverlayHandler");
            renderExhaustionHandle = MethodHandles.publicLookup()
                    .findStatic(clazz, "renderExhaustion", renderExhaustionType);
            renderFoodOrHealthOverlayHandle = MethodHandles.publicLookup()
                    .findStatic(clazz, "renderFoodOrHealthOverlay", renderFoodOrHealthOverlayType);
        ...
        NeoForge.EVENT_BUS.addListener((RenderGuiOverlayEvent.Pre evt) -> onRenderGuiOverlay$Pre(evt,
                renderExhaustionHandle
        ));
        NeoForge.EVENT_BUS.addListener((RenderGuiOverlayEvent.Post evt) -> onRenderGuiOverlay$Post(evt,
                renderFoodOrHealthOverlayHandle,
                foodOverlayType
        ));
    }

    private static void onRenderGuiOverlay$Pre(final RenderGuiOverlayEvent.Pre evt, MethodHandle methodHandle) {
        if (evt.getOverlay().overlay() == NeoForgeRidingBarsHandler.FOOD_LEVEL_MOUNTED_GUI_OVERLAY) {
            ...
            if (minecraft.player.getVehicle() instanceof LivingEntity &&
                ...
                try {
                    methodHandle.invoke(gui,
                            evt.getGuiGraphics(),
                            evt.getPartialTick(),
                            evt.getWindow().getScreenWidth(),
                            evt.getWindow().getScreenHeight()
                    );
                } catch (Throwable e) {
    ...
    private static void onRenderGuiOverlay$Post(final RenderGuiOverlayEvent.Post evt, MethodHandle methodHandle, Object foodOverlayType) {
