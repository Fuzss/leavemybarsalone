...
/**
 * copied from AppleSkin (licensed under public domain), only change being RenderOverlayType is set to public instead of package private
 * wasn't able to find a way to change access on the enum (accesstransformer didn't apply properly), therefore everything has been copied
 * only used when AppleSkin itself is installed
 */
@OnlyIn(Dist.CLIENT)
