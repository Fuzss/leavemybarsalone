...
import fuzs.leavemybarsalone.mixin.client.accessor.ForgeIngameGuiAccessor;
...
public class MountHUDHandler {
    ...
    public static void init() {
        OverlayRegistry.registerOverlayAbove(ForgeIngameGui.FOOD_LEVEL_ELEMENT,  new ResourceLocation(LeaveMyBarsAlone.MOD_ID, "food_level_mounted").toString(), (gui, mStack, partialTicks, screenWidth, screenHeight) -> {
        ...
        OverlayRegistry.registerOverlayAbove(ForgeIngameGui.EXPERIENCE_BAR_ELEMENT, new ResourceLocation(LeaveMyBarsAlone.MOD_ID, "experience_bar_mounted").toString(), (gui, mStack, partialTicks, screenWidth, screenHeight) -> {
            if (!LeaveMyBarsAlone.CONFIG.client().experienceBar) return;
            final Minecraft minecraft = Minecraft.getInstance();
